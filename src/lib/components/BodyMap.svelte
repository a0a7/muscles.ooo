        ...
        <g id="Anterior_View_Muscles" data-name="Anterior View Muscles" transform="translate(0, -6)">
          ...
          <g id="Adductors_Abductors" role="button" tabindex="0"  data-name="Adductors/Abductors"  on:mouseover={(e) => handleMouseOver(e, "Hips")} on:mouseout={handleMouseOut} on:focus={(e) => handleMouseOver(e, "Hips")} on:blur={handleMouseOut}>
            <g>
              ...
              <path class="hips" d="M129.17,249.1c-7.52,21.61-8.99,54.91-10.46,82.75,2.56-17.71,8.9-39.19,15.47-54.95,3.88-20.3,6.52-13.7-5.02-27.8Z" fill={getColor(muscleReps['HIPS'] || 0, maxActivation)}/>
            </g>
            <g>
              <path class="hips" d="M245.05,227.81c-3.8,6.94-18.39,12.84-25.99,17.37-4.61,7.73-7.11,16-14.24,20.62-1.83,1.45-1.31,1.85.96,1.53,23.25-3.08,51.61-15.74,39.26-39.51ZM237.52,261.39c3.31,13.94-1.45,30.63-6.19,43.68-12.89,13.16-27.78,32.17-28.71,57.81-2.24-33.54-17.1-60.24,8.92-81.34,4.81-3.91,20.94-21.78,25.98-20.15ZM250.48,249.1c7.52,21.61,8.99,54.91,10.46,82.75-2.56-17.71-8.9-39.19-15.47-54.95-3.87-20.3-6.52-13.7,5.02-27.8Z" fill={getColor(muscleReps['HIPS'] || 0, maxActivation)}/>
              <path class="hips" d="M237.52,261.39c3.31,13.94-1.45,30.63-6.19,43.68-12.89,13.16-27.78,32.17-28.71,57.81-2.24-33.54-17.1-60.24,8.92-81.34,4.81-3.91,20.94-21.78,25.98-20.15ZM250.48,249.1c7.52,21.61,8.99,54.91,10.46,82.75-2.56-17.71-8.9-39.19-15.47-54.95-3.87-20.3-6.52-13.7,5.02-27.8Z" fill={getColor(muscleReps['HIPS'] || 0, maxActivation)}/>
...
{#if hoveredMuscle && tooltipPosition}
    ...
    <p class="m-0">
        # Reps as Primary: 
        {musclePrimaryReps[hoveredMuscle.toUpperCase()]?.toFixed(0).toString()?.replace(/\B(?=(\d{3})+(?!\d))/g, " ")}
    </p>
    <p class="m-0">
        # Reps as Secondary: 
        {muscleSecondaryReps[hoveredMuscle.toUpperCase()]?.toFixed(0).toString()?.replace(/\B(?=(\d{3})+(?!\d))/g, " ")}
    </p>
    <p class="m-0">
        Relative Activation: {muscleReps[hoveredMuscle.toUpperCase()] > 0 ? ((muscleReps[hoveredMuscle.toUpperCase()] / maxActivation) * 100).toFixed(2) : 0}%
    </p>
    <p class="m-0">
        Absolute Activation: {muscleReps[hoveredMuscle.toUpperCase()] > 0 ? ((muscleReps[hoveredMuscle.toUpperCase()] / totalVolume) * 100).toFixed(2) : 0}%
    </p>
    </div>
{/if}
