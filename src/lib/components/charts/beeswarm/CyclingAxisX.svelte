<script>
    ...
    $: if (isWeekday) {
        ...
        tickVals = $xScale.ticks(ticks / 2); // Reduce the number of ticks for distance
    }
    ...
    function formatDistanceTick(tick) {
        const metric = get(useMetric);
        return metric 
            ? `${(tick / 1000).toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 0 })} km` 
            : `${(tick / 1609.34).toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 0 })} mi`;
    }
