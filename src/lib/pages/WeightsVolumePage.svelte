<script lang="ts">
    ...
    const muscleActivation = allMuscles.reduce((acc, muscle) => {
        acc[muscle] = 0;
        return acc;
    }, {} as { [key: string]: number });

    const exerciseSets: { name: string; reps: number; weight: number; time: string }[] = [];
    let exerciseMuscleMap: { [key: string]: { primaryMuscles: string[], secondaryMuscles: string[] } } = {};
    ...
    onMount(async () => {
            ...
            if (Array.isArray(activity.fullExerciseSets)) {
                activity.fullExerciseSets.forEach((exerciseSet: any) => {
                    exerciseSet.exercises.forEach((exercise: any) => {
                        ...
                        const muscles = exerciseMuscleMap[exercise.category];
                        if (muscles) {
                            muscles.primaryMuscles.forEach((muscle: string) => {
                                muscleActivation[muscle] += exerciseSet.repetitionCount || 0;
                            });
                            muscles.secondaryMuscles.forEach((muscle: string) => {
                                muscleActivation[muscle] += (exerciseSet.repetitionCount || 0) * 0.5;
                            });
                        }
                    });
                });
        ...
        if (svgElement) {
            allMuscles.forEach((muscle) => {
                ...
                muscleElements.forEach((muscleElement: any) => {
                    muscleElement.addEventListener("mouseenter", (event: any) => handleMouseEnter(muscle, event as MouseEvent));
                    muscleElement.addEventListener("mousemove", (event: any) => handleMouseMove(event as MouseEvent));
                    muscleElement.addEventListener("mouseleave", handleMouseLeave);
                    muscleElement.addEventListener("click", () => handleClick(muscle));
                    muscleElement.style.fill = getColor(muscleActivation[muscle], maxActivation);
                });
            });
        }
    });
    ...
    function handleMouseEnter(muscle: string, event: MouseEvent) {
        const svgRect = svgRef?.getBoundingClientRect();
        if (svgRect) {
            hoveredMuscle = muscle;
            tooltipPosition = { x: event.clientX - svgRect.left, y: event.clientY - svgRect.top };
        }
    }
...
<div class="relative w-full items-center justify-center">
    {#if hoveredMuscle && tooltipPosition}
        <div
            style="position: absolute; top: {tooltipPosition.y}px; left: {tooltipPosition.x}px; background-color: rgba(255, 255, 255, 0.9); padding: 8px; border-radius: 5px; box-shadow: 0px 4px 6px rgba(0, 0, 0, 0.1); z-index: 1000; transform: translate(10%, -110%); font-size: 14px; font-weight: 500; color: #333; white-space: nowrap;">
            <h4 style="margin: 0 0 5px 0; font-size: 14px; font-weight: bold;">{hoveredMuscle}</h4>
            <p style="margin: 0; font-size: 12px;">
                {volumeType == "weight" ? "Weight: " : "# Sets: "}
                {muscleActivation[hoveredMuscle]?.toString()?.replace(/\B(?=(\d{3})+(?!\d))/g, " ")} {volumeType == "weight" ? " " + weightUnit : ""}
            </p>
            <p style="margin: 0; font-size: 12px;">
                Relative Activation: {muscleActivation[hoveredMuscle] > 0 ? ((muscleActivation[hoveredMuscle] / maxActivation) * 100).toFixed(2) : 0}%
            </p>
            <p style="margin: 0; font-size: 12px;">
                Absolute Activation: {muscleActivation[hoveredMuscle] > 0 ? ((muscleActivation[hoveredMuscle] / totalVolume) * 100).toFixed(2) : 0}%
            </p>
        </div>
    {/if}
</div>